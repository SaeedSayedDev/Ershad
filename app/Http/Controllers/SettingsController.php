...
class SettingsController extends Controller
{
    ...
    function fetchGlobalSettings(Request $request)
    {
        ...
        $tax = Taxes::where('status', 1)->where('tax_title', 'urgent appointment tax')->first();
        // $taxes = Taxes::where('status', 1)->get();
        // $settings->taxes = $taxes;
        if ($request->has('user_id')) {
            $country_code = Users::where('id', $request->user_id)->first()?->country_code;
            if ($country_code != 20) {
        ...
        $settings->urgent_appointment_tax = $tax;
    ...
    function updateGlobalSettings(Request $request)
    {
        ...
        $settings->urgent_comission = $request->urgent_comission;
        $settings->cancelation_policy = $request->cancelation_policy;
    ...
    function fetchDoctorCatsList(Request $request)
    {
        ...
        foreach ($result as $item) {
            if ($item->parent != 0)
                $main_cat = DoctorCategories::where('is_deleted', 0)->where('id', $item->parent)->orderBy('id', 'DESC')->first()->title;
            else
            ...
            $edit = '<a data-icon="' . $imgUrl . '" data-title="' . $item->title . '" data-parent="' . $item->parent . '" data-disc="' . $item->disc . '" href="" class="mr-2 btn btn-primary text-white edit" rel=' . $item->id . ' >' . __("Edit") . '</a>';
            $delete = '<a href="" class="mr-2 btn btn-danger text-white delete" rel=' . $item->id . ' >' . __("Delete") . '</a>';
            $action =  $edit . $delete;
    ...
    {
        $coupon = Interests::find($id);
        $coupon->delete();
        return GlobalFunction::sendSimpleResponse(true, 'coupon deleted successfully!');
    }
    function editInterestItem(Request $request)
    {
        $path = GlobalFunction::saveFileAndGivePath($request->file);
    ...
    function addInterestItem(Request $request)
    {
        $rules = [
            'file' => 'required',
        ];
    ...
    function fetchAllInterestsList(Request $request)
    {
        $totalData =  Interests::count();
        $rows = Interests::orderBy('id', 'DESC')->get();
        $settings = GlobalSettings::first();
        ...
        $totalFiltered = $totalData;
        if (empty($request->input('search.value'))) {
            $result = Interests::offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
        } else {
            $search = $request->input('search.value');
            $result =  Interests::where(function ($query) use ($search) {
                $query->Where('name', 'LIKE', "%{$search}%");
            })->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
            $totalFiltered = Interests::where(function ($query) use ($search) {
                $query->Where('name', 'LIKE', "%{$search}%");
            })->count();
        }
        $data = array();
        foreach ($result as $item) {
            $edit = '<a data-heading="' . $item->img . '" data-interest="' . $item->name . '" href="" class="mr-2 btn btn-primary text-white edit" rel=' . $item->id . ' >' . __("Edit") . '</a>';

            $delete = '<a href="" class="mr-2 btn btn-danger text-white delete" rel=' . $item->id . ' >' . __("Delete") . '</a>';
            $action = $edit  . $delete;
            $imgUrl = GlobalFunction::createMediaUrl($item->img);
    ...
    function fetchAllInterests(Request $request)
    {
        $totalData =  Interests::count();
        $rows = Interests::orderBy('id', 'DESC')->get();
        $result = $rows;
        return GlobalFunction::sendDataResponse(true, 'Interests fetched success', $result);
    }
    function interests()
    {
        $settings = GlobalSettings::first();
    ...
    function deleteSlider($id)
    {
        ...
        $slider->delete();
        return GlobalFunction::sendSimpleResponse(true, 'slider deleted successfully!');
    }
    function editSliderItem(Request $request)
    {
        $path = GlobalFunction::saveFileAndGivePath($request->file);
        ...
        return GlobalFunction::sendSimpleResponse(true, 'slider edited successfully!');
    }
    function addSliderItem(Request $request)
    {
        $rules = [
            'file' => 'required',
        ];
    ...
    function fetchAllSlidersList(Request $request)
    {
        $totalData =  Sliders::count();
        $rows = Sliders::orderBy('id', 'DESC')->get();
        $settings = GlobalSettings::first();
        ...
        $totalFiltered = $totalData;
        if (empty($request->input('search.value'))) {
            $result = Sliders::offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
        } else {
            $search = $request->input('search.value');
            $result =  Sliders::where(function ($query) use ($search) {
                $query->Where('name', 'LIKE', "%{$search}%");
            })->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
            $totalFiltered = sliders::where(function ($query) use ($search) {
                $query->Where('name', 'LIKE', "%{$search}%");
            })->count();
        }
        $data = array();
        foreach ($result as $item) {
            $edit = '<a data-heading="' . $item->img . '" data-title="' . $item->title . '"data-link="' . $item->link . '" href="" class="mr-2 btn btn-primary text-white edit" rel=' . $item->id . ' >' . __("Edit") . '</a>';
    ...
    function fetchAllSliders(Request $request)
    {
        $totalData =  Sliders::count();
        $rows = Sliders::orderBy('id', 'DESC')->get();
        $result = $rows;
        return GlobalFunction::sendDataResponse(true, 'Sliders fetched success', $result);
    }
    function Sliders()
    {
        $settings = GlobalSettings::first();
    ...
    function deleteJobTitles($id)
    {
        $jobTitle = JobTitles::find($id);
        $jobTitle->delete();
        return GlobalFunction::sendSimpleResponse(true, 'JobTitle deleted successfully!');
    }
    function editJobTitlesItem(Request $request)
    {
        $jobTitle = JobTitles::find($request->id);
        $jobTitle->name = $request->name;
        $jobTitle->save();

        return GlobalFunction::sendSimpleResponse(true, 'JobTitle edited successfully!');
    }
    function addJobTitlesItem(Request $request)
    {
        $rules = [
            'name' => 'required',
        ];
    ...
    function fetchAllJobTitlesList(Request $request)
    {
        $totalData =  JobTitles::count();
        $rows = JobTitles::orderBy('id', 'DESC')->get();
        $settings = GlobalSettings::first();
        ...
        $totalFiltered = $totalData;
        if (empty($request->input('search.value'))) {
            $result = JobTitles::offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
        } else {
            $search = $request->input('search.value');
            $result =  JobTitles::where(function ($query) use ($search) {
                $query->Where('name', 'LIKE', "%{$search}%");
            })->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
            $totalFiltered = JobTitles::where(function ($query) use ($search) {
                $query->Where('name', 'LIKE', "%{$search}%");
            })->count();
        }
        $data = array();
        foreach ($result as $item) {
            $edit = '<a data-name="' . $item->name . '" href="" class="mr-2 btn btn-primary text-white edit" rel=' . $item->id . ' >' . __("Edit") . '</a>';
    ...
    function fetchAllJobTitles(Request $request)
    {
        $totalData =  JobTitles::count();
        $rows = JobTitles::orderBy('id', 'DESC')->get();
        $result = $rows;
        return GlobalFunction::sendDataResponse(true, 'JobTitles fetched success', $result);
    }
    function JobTitles()
    {
        $settings = GlobalSettings::first();
