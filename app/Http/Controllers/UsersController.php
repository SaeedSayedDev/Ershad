...
class UsersController extends Controller
{
    ...
    public function create_url(Request $request)
    {
        ...
        $signedUrl = URL::signedRoute('api.user.SucessaddMoneyToUserWallet', [
            'user_id' => $request->user_id,
            'amount' => $request->amount,
        ]);
        return $signedUrl;
    }
    public function SucessaddMoneyToUserWallet(Request $request)
    {
        if (!$request->hasValidSignature()) {
            return response()->json(['status' => false, 'message' => "signed!"]);
        }
        $user = Users::find($request->user_id);
        if ($user == null) {
            return response()->json(['status' => false, 'message' => "User doesn't exists!"]);
        ...
        GlobalFunction::addUserStatementEntry(
            ...
            Constants::credit,
            Constants::deposit,
            'monty added to wallet by fawaterk'
        );
        // Recharge Wallet History
        $rechargeLog = new UserWalletRechargeLogs();
        $rechargeLog->user_id = $user->id;
        $rechargeLog->amount = $request->amount;
        $rechargeLog->gateway = 7;
        $rechargeLog->transaction_id = GlobalFunction::generateTransactionId();
        $rechargeLog->transaction_summary = 'monty added to wallet by fawaterk';
        $rechargeLog->save();
    ...
    function fetchHomePageData(Request $request)
    {
        ...
        $user = Users::find($request->user_id);
        if ($user == null && $request->user_id != -1) {
            return GlobalFunction::sendSimpleResponse(false, 'User does not exists !');
        }
        ...
        if ($request->user_id != -1) {
            $appointments = Appointments::where('user_id', $user->id)->where('date', $request->date)
                ->with(['doctor', 'documents'])->whereIn('status', [Constants::orderAccepted, Constants::orderPlacedPending, Constants::orderWittingPayment])->get();
        } else {
            $appointments = [];
        }
    ...
    function updateUserDetails(Request $request)
    {
        ...
        if ($request->has('email')) {
            $user->identity = GlobalFunction::cleanString($request->email);
        }
        if ($request->has('fullname')) {
